...
begin
  ...
  results := results || format('12|A cannot edit B allergens|0|%s', n);

  delete from public.inventory where household_id = household_b;
  get diagnostics n = row_count;
  results := results || format('13|A cannot clear B pantry|0|%s', n);

  -- Positive control: a policy set that denies everything is not a passing
  -- policy set. Assertions 9-13 above only mean something if A can still
  -- write their own household's pantry.
  insert into public.inventory (household_id, ingredient_id, source)
  values ((select household_id from public.profiles where id = user_a), 'flour', 'manual');
  get diagnostics n = row_count;
  results := results || format('14|Positive control: A CAN stock own pantry|1|%s', n);
  ...
  select count(*) into n from public.inventory;
  results := results || format('15|Roommate SHARES the pantry|3|%s', n);

  select count(*) into n from public.user_preferences;
  results := results || format('16|Roommate sees only own preferences|1|%s', n);

  select count(*) into n from public.user_preferences where 'peanut' = any(allergens);
  results := results || format('17|Roommate cannot read A allergens|0|%s', n);

  select count(*) into n from public.meal_feedback;
  results := results || format('18|Roommate cannot read A feedback|0|%s', n);

  select count(*) into n from public.meal_satiety;
  results := results || format('19|Roommate cannot read A satiety|0|%s', n);
  ...
  select count(*) into n from public.inventory;
  results := results || format('20|Anon sees no pantry|0|%s', n);

  select count(*) into n from public.user_preferences;
  results := results || format('21|Anon sees no preferences|0|%s', n);

  select count(*) into n from public.households;
  results := results || format('22|Anon sees no households|0|%s', n);

  select count(*) into n from public.meal_satiety;
  results := results || format('23|Anon sees no satiety|0|%s', n);
