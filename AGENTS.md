...
Launch: Aug 24, 2026. Current development version: 0.1.0; the first launch release is 1.0.0. Full specs in `docs/`.
...
- Recipes: **bundled catalog plus optional live expansion, both free.** TheMealDB JSON is bundled in
  `src/data/` (~300, offline, owned), generated by `tools/catalog/` (Python).
  Spoonacular is an optional live query for breadth (380k, 50 pts/day, nothing stored).
...
- Voice: deferred post-launch (Aug 22 decision) — tap-to-listen via
  `@react-native-voice/voice` when built

## Architecture rules

- **`src/engine/` is PURE.** No React, no imports from `src/lib/`, no I/O.
  It takes a `Recipe[]` and does not know or care which source supplied it.
...
- **Spoonacular is best-effort.** Call it only when the bundled catalog is thin AND
  quota < 40 points AND online AND not cached. HTTP 402 is a normal outcome:
  return an empty array and let the bundled catalog stand. Never show the user an error.
